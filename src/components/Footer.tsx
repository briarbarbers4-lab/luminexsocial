...
export default function Footer() {
  return (
    <footer className="bg-primary-dark relative">
      <div
        className="absolute top-0 left-0 right-0 h-px"
        style={{
          background: 'rgba(13, 33, 161, 0.3)',
      ...
      <div className="container mx-auto px-6 md:px-12 py-16">
            ...
            >
              <div className="flex gap-3">
                <input
                  ...
                  className="bg-transparent border-none outline-none text-soft-white placeholder-deep-purple font-inter text-sm w-48"
                />
          ...
          <div>
            ...
            <ul className="space-y-2">
              {footerLinks.services.map((link, index) => (
                <li key={index}>
                  <a href="#" className="font-inter text-deep-purple hover:text-royal-blue transition-colors text-sm">
                    {link}
                  </a>
                </li>
              ))}
        ...
        <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-deep-purple/20">
          <p className="font-inter text-deep-purple text-sm mb-4 md:mb-0">
          ...
          <div className="flex gap-4">
            {socialLinks.map((social, index) => (
              <a
                key={index}
                href="#"
                className="w-10 h-10 rounded-full bg-deep-purple/20 hover:bg-royal-blue flex items-center justify-center transition-colors group"
                aria-label={social.label}
              >
                <social.icon className="w-5 h-5 text-deep-purple group-hover:text-soft-white transition-colors" />
              </a>
            ))}
          </div>
        </div>
